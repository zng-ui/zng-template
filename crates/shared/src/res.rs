...
#[cfg(feature = "release")]
const L10N_TEMPLATE_TAR: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/pack-l10n/l10n-template.tar"));
...
pub fn extract_l10n(dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tmp_template_tar = dir.with_added_extension(".tmp-template.tar");
    let _cleanup = zng::app::RunOnDrop::new(zng::clmv!(tmp_tar, tmp_template_tar, tmp_dir, || {
        let _ = std::fs::remove_file(tmp_tar);
        let _ = std::fs::remove_file(tmp_template_tar);
        let _ = std::fs::remove_dir_all(tmp_dir);
    }));

    std::fs::create_dir_all(&tmp_dir)?;
    std::fs::write(&tmp_tar, L10N_TAR)?;
    std::fs::write(&tmp_template_tar, L10N_TEMPLATE_TAR)?;

    for file in [tmp_tar, tmp_template_tar] {
    ...
    std::fs::rename(tmp_dir.join("l10n-template"), dir.join("template"))?;
